...
import { MoreHorizontal, Eye, Tag, Calendar, Weight, Building2, Edit } from "lucide-react"
...
interface GanadoListTableProps {
  ...
  onImageUpdate?: (ganadoId: string, newImageUrl: string) => void
}

export function GanadoListTable({
  ganado,
  pagination,
  onPageChange,
  onViewDetails,
  onImageUpdate,
}: GanadoListTableProps) {
  ...
  const handleImageUpdateLocal = (newImageUrl: string) => {
    if (selectedImageGanado && onImageUpdate) {
      onImageUpdate(selectedImageGanado.id, newImageUrl)
    }
  }
  ...
  return (
    <>
                  ...
                  <div className="flex items-center gap-3">
                    <div className="relative group">
                      ...
                      <Button
                        size="sm"
                        variant="secondary"
                        className="absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity rounded-full w-9 h-9 p-0"
                      ...
                      <div className="flex flex-wrap gap-1">
                        {animal.premiosObtenidos.slice(0, 2).map((premio, idx) => {
                          const isGrandChampion =
                            premio.toLowerCase().includes("gran campeón") ||
                            premio.toLowerCase().includes("gran campeon")
      ...
      <ImageEditDialog
        ...
        entityType="ganado"
        onImageUpdate={handleImageUpdateLocal}
      />
    </>
  )
}
